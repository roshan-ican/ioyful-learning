...
import PropertyHighlights2 from "@/components/hotel-single/PropertyHighlights";
...
const HotelSingleV2Dynamic = () => {
  ...
  useEffect(() => {
    if (!id) {
      // Return loading UI when id is not available
      return;
    }

    // Find and set the hotel data
    const foundHotel = hotelsData.find((item: any) => item.id == id);
    if (foundHotel) {
      setHotel(foundHotel);
    }
  }, [id]);
